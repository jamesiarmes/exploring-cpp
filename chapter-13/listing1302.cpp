...
int main()
{
  ...
  // Find where *iter belongs in the already sorted porion of the vector.
  ...
  for (std::vector<int>::iterator iter{data.begin()}, end{data.end()}; iter != end; )
  {
    ...
    {
      // erase the out-of-position item, advancing the iter at the same time.
      iter = data.erase(iter);
      // re-insert the value at the current position.
